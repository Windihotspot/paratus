<template>
  ...
  <apexchart
    ...
    height="300"
    :options="{
      labels: appChartData.status_breakdown?.map(s => s.status),
      legend: { position: 'bottom' },
      title: { text: 'Application Status Distribution' }
    }"
    :series="appChartData.status_breakdown?.map(s => s.count)"
  />
</div>
...
<div class="bg-white p-3 rounded shadow">
  <apexchart
    ...
    :options="{
      xaxis: {
        categories: appChartData.monthly_trend?.map(m => m.month)
      },
      title: { text: 'Application Trend' },
      colors: ['#3B82F6']
    }"
    :series="[
      {
        name: 'Applications',
        data: appChartData.monthly_trend?.map(m => m.total)
      }
    ]"
  />
</div>
...
<div class="bg-white p-3 rounded shadow">
  <apexchart
    ...
    :options="{
      xaxis: {
        categories: appChartData.daily_trend?.map(d => d.day)
      },
      stroke: { curve: 'smooth' },
      title: { text: 'Daily Applications' }
    }"
    :series="[
      {
        name: 'Applications',
        data: appChartData.daily_trend?.map(d => d.total)
      }
    ]"
  />
</div> -->
